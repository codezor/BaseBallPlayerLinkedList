...
//Declare PlayerList
	...
	{
		Node *pPreviousLocation;			//trailing pointer loacation
		Node *pNewPlayer;					//new player node
		pCurrent = pFirst;					// Set current location pointer to first location
		pPreviousLocation = NULL;			// Set previous location to NULL
		...
		{
			pPreviousLocation = pCurrent;		//move 
			pCurrent = pCurrent->pNext;
		}
				
		if (pPreviousLocation == NULL)
		{
		...
		else
		{
			...
			pPreviousLocation->pNext = pNewPlayer;
	...
	void PlayerList::remove(Player &newPlayer)
	{
		...
		{
			pPreviousLocation = pCurrent;
			pCurrent = pCurrent->pNext;
		}
		// can add nested if to not remove if player is not found in the List
		if (pPreviousLocation == NULL) // First Node in the list
		{
			pFirst = pCurrent->pNext;
			pPreviousLocation->pNext = pCurrent->pNext;
			delete pCurrent;
		}

		if (pCurrent->pNext == NULL) //Last Node in the list
		{
			pLast = pPreviousLocation;
			delete pCurrent;
		}
	...
	bool PlayerList::findPlayer(Player &findPlayer)
	{
		...
		Node *pPreviousLocation;		// trailing pointer loacation
		pCurrent = pFirst;				// Set current location pointer to first location
		pPreviousLocation = pCurrent;	// Set previous location to NULL
